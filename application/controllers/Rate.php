...
class Rate extends CI_Controller {
    ...
    public function index() {
        ...
        $query = $this->Md->query("SELECT *,users.name AS user,users.image AS userImage FROM rate LEFT JOIN users ON rate.userID  = users.Id");
        if ($query) {
            $data['c'] = $query;
        } else {
            $data['c'] = array();
        ...
        if (!empty($_POST)) {
            foreach ($_POST as $field_name => $val) {
                ...
                if (!empty($id) && !empty($field_name) && !empty($val)) {
                    ...
                    $this->Md->update_dynamic($id, 'id', 'rate', $task);
                    echo "updated";
                } else {
