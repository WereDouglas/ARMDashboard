...
class Follow extends CI_Controller {
    ...
    public function index() {
        ...
        $query = $this->Md->query("SELECT *,users.name AS user,users.image AS userImage,customer.name as customer,customer.image AS customerImage,item.image AS itemImage,item.name AS product FROM follow LEFT JOIN users ON follow.UserID  = users.Id LEFT JOIN customer ON follow.CustomerID = customer.ID LEFT JOIN item ON follow.ItemID = item.Id");
        if ($query) {
            $data['d'] = $query;
        } else {
            $data['d'] = array();
        }
        $query = $this->Md->query("SELECT * FROM itemreview");
        if ($query) {
            $data['r'] = $query;
        } else {
            $data['r'] = array();
        }
        $query = $this->Md->query("SELECT * FROM itemstatus");
        if ($query) {
            $data['s'] = $query;
        } else {
            $data['s'] = array();
        }
        $this->load->view('view-follow', $data);
    }
