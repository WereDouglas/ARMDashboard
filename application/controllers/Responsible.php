...
class Responsible extends CI_Controller {
    ...
    public function index() {
        ...
        $query = $this->Md->query("SELECT *,users.name AS user,users.image AS userImage,customer.name as customer,customer.image AS customerImage FROM responsible LEFT JOIN users ON responsible.UserID  = users.Id LEFT JOIN customer ON responsible.CustomerID = customer.ID");
        if ($query) {
            $data['d'] = $query;
        } else {
            $data['d'] = array();
        ...
        $this->load->view('view-responsible', $data);
    }
    ...
    public function delete() {
        ...
        $query = $this->Md->cascade($id, 'responsible', 'id');
        $this->session->set_flashdata('msg', '<div class="alert alert-error">  <strong> Care deleted ' . '	</strong></div>');
        redirect('responsible', 'refresh');
    }
        ...
        if (!empty($_POST)) {
            foreach ($_POST as $field_name => $val) {
                ...
                if (!empty($id) && !empty($field_name) && !empty($val)) {
                    ...
                    $this->Md->update_dynamic($id, 'id', 'responsible', $task);
                    echo "updated";
                } else {
