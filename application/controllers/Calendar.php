...
class Calendar extends CI_Controller {
    ...
    public function index() {
      ...
      $query = $this->Md->query("SELECT * FROM schedule");
        if ($query) {
            $data['v'] = $query;
        } else {
            $data['v'] = array();
        }
        $query = $this->Md->query("SELECT * FROM schedule limit 4");
        if ($query) {
            $data['vs'] = $query;
        } else {
            $data['vs'] = array();
    ...
    public function create() {
        ...
        if ($name != "") {
            $br = array( 'id' => $this->GUID(), 'name' => $this->input->post('name'), 'location' => $this->input->post('location'), 'address' => $this->input->post('address'),'contact' => $this->input->post('contact'), 'orgID' => $this->session->userdata('orgID'));
            $this->Md->save($br, 'branch');
