...
class Invoice extends CI_Controller {
    ...
    public function index() {
        ...
        $query = $this->Md->query("SELECT *,users.Name as user,customer.name as customer,vendor.name as Vendor FROM invoice LEFT JOIN users ON invoice.UserID  = users.Id LEFT JOIN vendor ON invoice.vendorID = vendor.id LEFT JOIN customer ON invoice.customerID = customer.id");
    ...
    public function create() {
        ...
        $query = $this->Md->query("SELECT * FROM invoice WHERE name='" . $this->input->post('name') . "'");
        if (count($query)) {
    ...
    public function exists() {
        ...
        if (!$get_result)
            echo '<span style="color:#f00"> This invoice <strong style="color:#555555" >' . $user . '</strong> does not exist in our database.' . '<a href= "' . $user . '" value="' . $user . '" id="myLink" style="background #555555;color:#0749BA;" onclick="NavigateToSite()">Click here to add </a></span>';
        else
    ...
    public function update_image() {
        ...
        $userID = $this->input->post('invoiceID');
        ...
        if (!$this->upload->do_upload($file_element_name)) {
            ...
            redirect('/invoice/profile/' . $namer, 'refresh');
        ...
        $this->Md->update_dynamic($userID, 'id', 'invoice', $user);
    ...
    public function delete() {
        ...
        $query = $this->Md->cascade($id, 'invoice', 'id');
        ...
        if (!empty($_POST)) {
            foreach ($_POST as $field_name => $val) {
                ...
                if (!empty($id) && !empty($field_name) && !empty($val)) {
                    ...
                    $this->Md->update_dynamic($id, 'id', 'invoice', $task);
                    echo "updated";
                } else {
