...
class Payment extends CI_Controller {
    ...
    public function create() {
        ...
        $query = $this->Md->query("SELECT * FROM payment WHERE name='" . $this->input->post('name') . "'");
        if (count($query)) {
    ...
    public function exists() {
        ...
        if (!$get_result)
            echo '<span style="color:#f00"> This payment <strong style="color:#555555" >' . $user . '</strong> does not exist in our database.' . '<a href= "' . $user . '" value="' . $user . '" id="myLink" style="background #555555;color:#0749BA;" onclick="NavigateToSite()">Click here to add </a></span>';
        else
    ...
    public function update_image() {
        ...
        $userID = $this->input->post('paymentID');
        ...
        if (!$this->upload->do_upload($file_element_name)) {
            ...
            redirect('/payment/profile/' . $namer, 'refresh');
        ...
        $this->Md->update_dynamic($userID, 'id', 'payment', $user);
        ...
        if (!empty($_POST)) {
            foreach ($_POST as $field_name => $val) {
                ...
                if (!empty($id) && !empty($field_name) && !empty($val)) {
                    ...
                    $this->Md->update_dynamic($id, 'id', 'payment', $task);
                    echo "updated";
                } else {
