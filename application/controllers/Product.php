...
class Product extends CI_Controller {
    ...
    public function create() {
        ...
        $query = $this->Md->query("SELECT * FROM product WHERE name='" . $this->input->post('name') . "'");
        if (count($query)) {
    ...
    public function exists() {
        ...
        if (!$get_result)
            echo '<span style="color:#f00"> This product <strong style="color:#555555" >' . $user . '</strong> does not exist in our database.' . '<a href= "' . $user . '" value="' . $user . '" id="myLink" style="background #555555;color:#0749BA;" onclick="NavigateToSite()">Click here to add </a></span>';
        else
    ...
    public function update_image() {
        ...
        if (!$this->upload->do_upload($file_element_name)) {
            ...
            redirect('/product/profile/' . $namer, 'refresh');
        ...
        $this->Md->update_dynamic($userID, 'id', 'product', $user);
        ...
        if (!empty($_POST)) {
            foreach ($_POST as $field_name => $val) {
                ...
                if (!empty($id) && !empty($field_name) && !empty($val)) {
                    ...
                    $this->Md->update_dynamic($id, 'id', 'item', $task);
                    echo "updated";
                } else {
