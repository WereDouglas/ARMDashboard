...
class Instruction extends CI_Controller {
    ...
    public function index() {
        ...
        $query = $this->Md->query("SELECT *,users.name AS user,users.image AS userImage,customer.name as customer,customer.image AS customerImage,item.name AS product,item.image AS itemImage FROM instruction LEFT JOIN users ON instruction.UserID  = users.Id LEFT JOIN customer ON instruction.CustomerID = customer.ID LEFT JOIN item ON instruction.ItemID = item.Id");
        if ($query) {
            $data['d'] = $query;
        } else {
            $data['d'] = array();
        ...
        $this->load->view('view-instruction', $data);
    }
