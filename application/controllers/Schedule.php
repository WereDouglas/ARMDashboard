...
class Schedule extends CI_Controller {
    ...
    public function index() {
        ...
        $query = $this->Md->query("SELECT *,users.name AS user,users.image AS userImage,customer.name as customer,customer.image AS customerImage FROM schedule LEFT JOIN users ON schedule.UserID  = users.Id LEFT JOIN customer ON schedule.CustomerID = customer.ID");
        if ($query) {
            $data['d'] = $query;
        } else {
            $data['d'] = array();
        ...
        $this->load->view('view-schedule', $data);
    }
        ...
        if (!empty($_POST)) {
            foreach ($_POST as $field_name => $val) {
                ...
                if (!empty($id) && !empty($field_name) && !empty($val)) {
                    ...
                    $this->Md->update_dynamic($id, 'id', 'schedule', $task);
                    echo "updated";
                } else {
