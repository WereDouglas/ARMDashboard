...
class Transaction extends CI_Controller {
    ...
    public function index() {
        ...
        $query = $this->Md->query("SELECT *,item.name as itemID FROM transaction LEFT JOIN item ON transaction.ItemID  = item.Id");
        ...
        $this->load->view('view-transaction', $data);
    }

    public function add() {

        $this->load->view('add-transaction');
    }
    ...
    public function create() {
        ...
        $query = $this->Md->query("SELECT * FROM transaction WHERE name='" . $this->input->post('name') . "'");
        if (count($query)) {
    ...
    public function exists() {
        ...
        if (!$get_result)
            echo '<span style="color:#f00"> This transaction <strong style="color:#555555" >' . $user . '</strong> does not exist in our database.' . '<a href= "' . $user . '" value="' . $user . '" id="myLink" style="background #555555;color:#0749BA;" onclick="NavigateToSite()">Click here to add </a></span>';
        else
    ...
    public function update_image() {
        ...
        $userID = $this->input->post('transactionID');
        ...
        if (!$this->upload->do_upload($file_element_name)) {
            ...
            redirect('/transaction/profile/' . $namer, 'refresh');
        ...
        $this->Md->update_dynamic($userID, 'id', 'transaction', $user);
    ...
    public function delete() {
        ...
        $query = $this->Md->cascade($id, 'transaction', 'id');
        ...
        if (!empty($_POST)) {
            foreach ($_POST as $field_name => $val) {
                ...
                if (!empty($id) && !empty($field_name) && !empty($val)) {
                    ...
                    $this->Md->update_dynamic($id, 'id', 'transaction', $task);
                    echo "updated";
                } else {
